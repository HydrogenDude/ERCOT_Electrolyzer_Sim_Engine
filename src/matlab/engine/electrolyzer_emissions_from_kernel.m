function emissions = electrolyzer_emissions_from_kernel(inputs, emissions, sim_results)
...
% Required signals
% -------------------------------
P_system_kW = sim_results.P_system_kW(:);

assert(isfield(emissions, 'alpha_kg_per_MWh'), ...
    'Emissions kernel missing: alpha_kg_per_MWh');

alpha = emissions.alpha_kg_per_MWh(:);

assert(length(P_system_kW) == length(alpha), ...
    'Mismatch: electrolyzer power and emissions kernel length');

% -------------------------------
% Convert power to energy
% -------------------------------
% kW × hr → kWh → MWh
energy_MWh = (P_system_kW .* dt_hr) / 1000;
